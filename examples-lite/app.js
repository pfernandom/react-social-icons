...
import {
  SocialIcon,
  SocialIconDatabase,
} from '../src/react-social-icons-lite.js'

SocialIconDatabase.importNetwork(facebook).importNetwork(github)
...
function Page() {
  return (
    <>
      ...
      <p>
        If you have a need to specify the network, you can. If you don't specify
        a url, your <code>href</code> attribute will be omitted. You can include
        props for both <code>url</code> and <code>network</code>. The{' '}
      ...
      <p>
        ...
        network as an icon's accessible label. If you think the social network
        name is not enough context, you can pass in the <code>label</code>
        &nbsp;prop.
      </p>
      <div className="one-line-example">
...
React.render(<SocialIcon url="https://linkedin.com/in/jaketrent" />, document.body);`}</pre>
      ...
      <p>
        ...
        the <code>head</code>. This is great because you don't have to import
        any additional stylesheet to support this library. But more specificity
        in selectors will be required when overriding default styles from
